...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    // 1. Initial Quick Check (Stale-While-Revalidate) from cache
    const savedProfile = typeof window !== 'undefined' ? sessionStorage.getItem('sb-profile-cache') : null;
    if (savedProfile) {
      try {
        const parsed = JSON.parse(savedProfile);
        setProfile(parsed);
        // We don't set loading=false here yet, to ensure we verify the session
      } catch (e) {
        /* ignore */
      }
    }
    ...
    const { data: authListener } = supabase.auth.onAuthStateChange(async (event, session) => {
      if (session) {
        ...
        setUser(null);
        setProfile(null);
        if (typeof window !== 'undefined') sessionStorage.removeItem('sb-profile-cache');
      }
    });
  ...
  const fetchUserProfile = async (userId) => {
    try {
      ...
      if (typeof window !== 'undefined') {
        sessionStorage.setItem('sb-profile-cache', JSON.stringify(normalizedProfile));
      }
      setError(null);
