        ...
        supabase.auth.signUp({
          ...
          options: {
            data: {
              ...
              department: additionalData.department_name || null,
            },
          },
        }),
...
export const logoutUser = async () => {
  ...
  if (typeof window !== "undefined") {
    ...
    sessionStorage.clear(); // Clear all caches (including sb-profile-cache)
  }
  return { success: true };
};
...
export const resetPasswordRequest = async (email) => {
  try {
    const { error } = await supabase.auth.signInWithOtp({
      ...
      options: {
        shouldCreateUser: false, // Ensure we only send to existing users
      },
    });
...
export const verifyOtpCode = async (email, token) => {
  try {
    const { data, error } = await supabase.auth.verifyOtp({
      email: email.trim().toLowerCase(),
      token,
      type: 'email' // Or 'magiclink' depending on Supabase settings, but 'email' is for 6-digit codes
    });
    if (error) throw error;
