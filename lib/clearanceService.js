...
const withTimeout = (promise, ms = 45000) => {
  ...
  const timeout = new Promise((_, reject) => {
    timeoutId = setTimeout(() => {
      reject(new Error("Database request timed out. This usually happens due to a poor internet connection or a database lock. Please refresh and try again."));
    }, ms);
  });
  ...
  try {
    ...
    try {
      ...
      for (const payload of insertCandidates) {
        // Do NOT wrap the critical write in custom timeout; let Supabase return authoritative result.
        ...
        if (!isMissingColumnError(requestError)) break;
      }
    } catch (timeoutOrNetworkError) {
      const latestAfterTimeout = await withTimeout(getLatestRequest(), 20000);
      const justCreated =
        latestAfterTimeout &&
        new Date(latestAfterTimeout.created_at).getTime() >= submitStartedAt - 8000 &&
    ...
    if (requestError) {
      ...
      throw new Error(
        `Failed to create clearance request: ${requestError.message}${requestError.code ? ` (code: ${requestError.code})` : ""}`
      );
    }
    ...
    // 2/3. Run non-critical follow-up in background so UI does not wait.
    void (async () => {
      try {
        // Preferred sync path: DB-side function (security definer) to populate status rows.
        const { error: rpcError } = await withTimeout(
          supabase.rpc("sync_clearance_status_for_request", { p_request_id: requestData.id }),
          12000
        );

        if (rpcError) {
          const rpcMessage = String(rpcError?.message || "").toLowerCase();
          const rpcNotFound = rpcMessage.includes("does not exist") || rpcMessage.includes("not found");

          if (!rpcNotFound) {
            console.warn("Status sync RPC warning:", rpcError?.message || rpcError);
          }

          // Fallback path: insert only missing rows from client side.
          ...
          if (depts && depts.length > 0) {
            ...
            if (existingErr) {
              console.warn("Existing status lookup warning:", existingErr?.message || existingErr);
            }

            const existingSet = new Set((existing || []).map((r) => r.department_id));
            const missingEntries = depts
            ...
            if (missingEntries.length > 0) {
              const { error: statusError } = await withTimeout(
                supabase.from("clearance_status").insert(missingEntries),
                12000
              );
              if (statusError) {
                console.error("Error creating fallback status entries:", statusError);
              }
            }
          }
        }
    ...
    console.warn("SubmitClearanceRequest warning:", error?.message || error);
    return {
      success: false,
      error: error?.message || "Failed to submit clearance request.",
 ...
 * @param {boolean} options.includeDocuments - Whether to fetch attached documents (default true)
 * @param {number} options.requestTimeoutMs - Timeout for primary query in milliseconds (default 30000)
 */
export const getStudentClearances = async (studentId, includeExpired = false, options = {}) => {
  try {
    ...
    let allDocuments = [];
    if (includeDocuments && requestIds.length > 0) {
      try {
        ...
        console.warn("Documents fetch skipped due to timeout/error:", docErr?.message || docErr);
        allDocuments = [];
      }
    }
